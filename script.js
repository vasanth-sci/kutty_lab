...
const REVERSED_SCALES = {
    Viridis: [[0,"#fde725"],[0.1,"#b5de2b"],[0.2,"#6ece58"],[0.3,"#35b779"],[0.4,"#1f9e89"],[0.5,"#26828e"],[0.6,"#31688e"],[0.7,"#3e4989"],[0.8,"#482878"],[0.9,"#440154"],[1,"#440154"]],
    Jet: [[0,"#ff0000"],[0.2,"#ff7f00"],[0.4,"#ffff00"],[0.6,"#7fff7f"],[0.8,"#007fff"],[1,"#00007f"]],
...
function plotScalar(expr, bg, txt, colorScale) {
    ...
    if (vars.size <= 1) {
        ...
        Plotly.react("scalarPlot",[{x,y,type:'scatter',mode:'lines',line:{color:'#6366f1',width:3}}], {...layout,xaxis:{range:scalar1DBounds.x,autorange:false,color:txt},yaxis:{color:txt}});
        ...
        Plotly.react("scalarPlot",[{
            ...
            marker:{size:currentSize,color:C,colorscale:colorScale,showscale:true,opacity:0.8}
        ...
        for(let i=0;i<y.length;i++){
            let row=[];
            for(let j=0;j<x.length;j++){ try{row.push(math.evaluate(expr,{x:x[j],y:y[i]}));}catch(e){row.push(0);} }
            Z.push(row);
        }
        Plotly.react("scalarPlot",[{ type:"surface",x,y,z:Z,colorscale:colorScale,showscale:true }],{...layout,scene:{aspectmode:'cube',xaxis:{color:txt},yaxis:{color:txt},zaxis:{color:txt}}});
    }
}
